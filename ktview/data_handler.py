...
class DataHandler(object):
    ...
    def  __init__(self, file=file, file_by_name=True):
        ...
        try:
            ...
            for row in data:
                row[0] = self._millisecond_timedelta(start_time, datetime.strptime(row[0], DATE_FORMAT))
                row[1:] = map(float, row[1:])
            self.data = zip(*data)
        except (AttributeError, IOError):
            self.headers, self.data = [], []

    def _millisecond_timedelta(self, start, end):
